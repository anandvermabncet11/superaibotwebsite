...
const UserDetail = () => {
  ...
  const handleLimitUpdate = async (userId) => {
    ...
    try {
      ...
      Swal.fire({
        icon: response?.data?.msg ? "success" : "",
  ...
  const handleCpFun = async (userId) => {
    try {
      const req = {
        user_id: userId,
      };
      const response = await apiConnectorPost(endpoint.handle_cp_fun, req);
      Swal.fire({
        icon: response?.data?.msg ? "success" : "",
        title: response?.data?.msg ? "Success" : "",
        text: response?.data?.msg || "Something happened",
      });
      client.refetchQueries("get_user_admin");
    } catch (error) {
      console.error("Error updating limit:", error);
      alert("Something went wrong.");
    }
  ...
  const tablerow = allData?.data?.map((row, index) => {
    return [
      ...
      <span>{row?.jnr_achieve_reward ? row?.jnr_achieve_reward : "--"}</span>,
      <span>{row?.lgn_real_name ? row?.lgn_real_name : "--"}</span>,
      ...
      <span>
        <FormControlLabel
          control={
            <Switch
              onChange={() => handleCpFun(row.lgn_jnr_id)}
              checked={row.jnr_is_active_for_income  === "Active"}
            />
          }
        />
      </span>,
      ...
      //     value={
      //       limits[row.lgn_jnr_id] ??
      //       Number(row.jnr_mlm_with_cap).toFixed(2) ??
      //       ""
      //     }
      //     onChange={(e) =>
      //       setLimits((prev) => ({
