...
const Profile = () => {
  ...
  const fkTeam = useFormik({
    initialValues: {
      user_id: "",
      topup_amnt: user_profile?.jnr_team_topup_bal || "",
    },
    enableReinitialize: true,
    onSubmit: () => {
      const reqbody = {
        user_id: fkTeam.values.user_id,
        topup_amnt: fkTeam.values.topup_amnt,
      };
      TopupTeamFn(reqbody);
    },
  });

  async function TopupTeamFn(reqbody) {
    setLoding(true);
    try {
      const res = await apiConnectorPost(
        endpoint?.topup_team_id_by_leader,
        reqbody
      );
      toast.success(res?.data?.message);
      Swal.fire({
        title: String(res?.data?.success) === "true" ? " Congratulations!" : "",
        text: res?.data?.message,
        confirmButtonColor: "black",
      });
      if (res?.data?.success) {
  ...
  async function getSponName() {
    try {
      const res = await apiConnectorGet(
        endpoint?.get_spon_name + `?userid=${fkTeam.values.user_id}`
      );
      setSponName(res?.data?.result?.[0]?.lgn_real_name || "Invalid name");
    } catch (e) {
      toast(e?.message || "Invalid ID");
    }
  }
  useEffect(() => {
    getSponName();
  }, [fkTeam.values.user_id]);
          ...
          <div className="bg-gray-800 p-3 rounded-lg shadow-lg">
            ...
            <div className="space-y-3 text-sm text-gray-300">
              ...
              <div className="flex flex-col  justify-between py-1 border-b border-gray-700">
                ...
                <span className="bg-gradient-to-r from-yellow-300 via-orange-500 to-gray-100 bg-clip-text text-transparent text-[8px]">
              ...
              {user_profile?.jnr_allow_for_team_topup && (
                <>
                  <div className="flex justify-between py-1">
                    <span>Topup Team Balance: </span>
                    <span className={`font-medium  `}>
                      {user_profile?.jnr_team_topup_bal}
                    </span>
                  </div>
                  <div className="flex justify-between py-1">
                    <span>Topup Team Id: </span>
                    <Button
      ...
      {showTopupModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-gray-900 p-6 rounded-lg shadow-lg w-full max-w-md mx-auto relative text-gray-100">
            ...
            {/* Adjusted background and width */}
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold text-white"> Topup Team </h3>
              <button
                className="text-gray-400 hover:text-white"
                onClick={() => {
            ...
            <form onSubmit={fkTeam.handleSubmit}>
              <div className="mb-4">
                <label
                  htmlFor="user_id"
                  className="block text-sm font-medium text-gray-300 mb-1"
                ...
                <input
                  ...
                  id="user_id"
                  className="w-full px-4 py-2 rounded bg-gray-800 text-white border border-gray-700 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  value={fkTeam.values.user_id}
                  onChange={fkTeam.handleChange}
                />
                <span>{sponName} </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label
                    ...
                    className="block text-sm font-medium text-gray-300 mb-1"
                  >
                    Topup Amount <span className="text-red-500">* </span>
                  </label>
                  <input
                    type="number"
                    name="topup_amnt"
                    id="topup_amnt"
                    className="w-full px-4 py-2 rounded bg-gray-800 text-white border border-gray-700 focus:outline-none focus:ring-1 focus:ring-blue-500"
                    value={fkTeam.values.topup_amnt}
                    onChange={fkTeam.handleChange}
                  />
                </div>
              </div>
